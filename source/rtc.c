...
bool initRtc()
{
	...
	I2C_MasterInit(I2C_MASTER_BASEADDR, &masterConfig, CLOCK_GetFreq(I2C0_CLK_SRC));
...
bool setTimeTime(uint8_t * time, uint8_t len)
{
...
bool setTime(uint8_t hund, uint8_t sec, uint8_t min, uint8_t hour, uint8_t date, uint8_t month, uint16_t year, uint8_t day)
{
	...
	{
		set24Hour();
		status = setTimeTime(_time, TIME_ARRAY_LENGTH);
		set12Hour();
	}
	else
	{
...
bool setHundredths(uint8_t value)
{
	...
	return setTimeTime(_time, TIME_ARRAY_LENGTH);
}
...
bool setAlarmTime(uint8_t * alarmTime, uint8_t len)
{
	if (len != TIME_ARRAY_LENGTH)
		return false;
...
bool setAlarm(uint8_t sec, uint8_t min, uint8_t hour, uint8_t date, uint8_t month)
{
	...
	return setAlarmTime(alarmTime, TIME_ARRAY_LENGTH);
}
...
void setCountdownTimer(uint8_t duration, uint8_t unit, bool repeat, bool pulse)
{
...
uint8_t readRegister(uint8_t addr)
{
    ...
    uint8_t data;
	masterXfer.slaveAddress = RV1805_ADDR;
...
bool writeRegister(uint8_t addr, uint8_t val)
{
	...
	masterXfer.dataSize = 1;
	masterXfer.flags = kI2C_TransferDefaultFlag;
	return 	I2C_MasterTransferBlocking(I2C_MASTER_BASEADDR, &masterXfer) == kStatus_Success;
